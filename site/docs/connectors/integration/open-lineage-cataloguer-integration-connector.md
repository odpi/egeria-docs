...
The OpenLineage Cataloguer integration connector registers an OpenLineage listener with the integration daemon context and catalogues any processes described in the OpenLineage events it received that are not already known to the open metadata ecosystem.
...
This is its connection definition to use on the [administration commands that configure the integration daemon](/guides/admin/servers/configuring-an-integration-daemon).
