...
![Add Configuration](/education/tutorials/intellij-add-configuration.png)
...
![Configuration Templates](/education/tutorials/intellij-configuration-templates.png)
Scroll down to *Jar Application* template and select it.  A form appears on the right-hand side.
![Jar Application Template](/education/tutorials/intellij-jar-application-template.png)
Enter:

* `EgeriaPlatform` at the top under `Name:`
* `Path to Jar` is set to the directory where the OMAG Server Platform jar is located
* `VM Options` is set to `-Dstrict.ssl=false -Dloader.path=server/lib -Dserver.port=7443`.  This turns off SSL checking, points to the `lib` directory where you will be adding your new connector jar files and requests that the OMAG Server listens on port `7443`.
...
![OMAG Server Platform Configuration](/education/tutorials/intellij-omag-server-platform-configuration.png)
...
![EgeriaPlatform in configurations menu](/education/tutorials/intellij-egeria-platform-in-menu.png)
...
![Edit configurations](/education/tutorials/intellij-edit-configurations.png)
...
```bash
/Library/Java/JavaVirtualMachines/temurin-17.jdk/Contents/Home/bin/java -DString.ssl=false -Dloader.path=lib,extra -Dserver.port=7443 -Dfile.encoding=UTF-8 -jar /egeria-install/egeria-platform-5.0-distribution.tar.gz/assembly/platform/omag-server-platform-4.3.jar
...
2023-09-04T15:46:43.116+01:00  INFO 76773 --- [           main] o.s.b.w.embedded.tomcat.TomcatWebServer  : Tomcat initialized with port(s): 7443 (https)
...
2023-09-04T15:46:46.591+01:00  INFO 76773 --- [           main] o.s.b.w.embedded.tomcat.TomcatWebServer  : Tomcat started on port(s): 7443 (https) with context path ''
...
![Run tab control panel](/education/tutorials/intellij-run-tab-control-panel.png)
