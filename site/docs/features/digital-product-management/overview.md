...
The vocabularies of the two standards are registered as valid metadata values by the [Core Content Pack](/content-packs/core-content-pack/overview) for the properties in which the cataloguers store them, so a user interface can offer the standard choices when a product manager edits a data quality rule, a data field or a product.
