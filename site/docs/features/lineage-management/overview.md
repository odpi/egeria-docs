...
So design lineage describes the digital resources and their linkages that are known at deployment.  Some tools, such as ETL engines, produce design lineage as part of their design tools.  Other technologies rely on design lineage capture in the dev-ops pipeline or the automatic cataloguing of digital resources as they are added to the pre-production or production environment.
...
Some process cause resources to be created, moved and deleted.  This blurs the boundary between design lineage and operational lineage since the operational lineage needs to cause the cataloging of resources that were dynamically created or modified.  This is particularly true when processing with files. The next set of images (figure 6-10) show different patterns of lineage that can be chosen for particular circumstances.  The choice comes down to the value that the detail brings against the cost 
...
> **Figure 7:** New files are read and created each time the process runs; these are catalogued and linked to the process as soon as they are detected; however no operation lineage is captured so it is not possible to know which process instance created each file.  This may not matter if the process runs regularly and the creation times of the files are sufficient to correlate them with the processing.

![Figure 8](operational-lineage-files-no-destination-file-linkage.svg)
> **Figure 8:** New files are  catalogued in the destination - but only the folder that they belong to is linked to the process.  This is very efficient if all files in the folder come from the process.  However if multiple processes are populating the folder, there is no information on which process created each file.
...
> **Figure 10:** This final example shows no operation lineage.  The new files are just catalogued in their folder as they are created.

Each of these patterns is valuable for different reasons.  It is important to keep your use case in mind when designing lineage capture.  *More* is not always better.

### OpenLineage Standard

[OpenLineage](https://github.com/OpenLineage/OpenLineage) is a sister open source project to Egeria since it is also part of the [LF AI and Data Foundation](https://lfaidata.foundation/).  It defines a standard for [*operational lineage*](#design-lineage-verses-operational-lineage). This is a very important development since it offers the hope of standardization in the capture of operational lineage 
...
> **Figure 12:** a three step process

Figure 13 shows the events from this process.  Notice each event has an *eventType* that describes the type of processing that is being described.  The *runId* identifies the process instance.  The sub-processes are linked to the top-level process with the *parentRunId*.
...
- *eventTime* - the type of the event.
...
Throughout the RunEvent are `additionalProperties`.  These allow extensions to be added to the event.  These extensions are called *facets*.  The OpenLineage standard defines a number of standard facets and any organization or processing engine can define their own.
...
Any server can implement the OpenLineage REST API.  Figure 12 shows Marquez, the reference implementation of the standard, acting as the back end to receive OpenLineage events from a spark processing engine.
...
The OpenLineage project provides a simple implementation of the OpenLineage API called the *proxy backend*.  This is designed to act as a side car to the processing engine that can distribute the OpenLineage events to multiple external consumers through a kafka topic.
...
Egeria offers two approaches to capture OpenLineage events from the processing engines.  The first (figure 22) listens on the kafka topic(s) populated by the proxy backends tied to each of the processing engines.
...
Egeria's [integration daemon](/egeria-docs/concepts/integration-daemon) also supports the OpenLineage API for local processing engines.  It is one of the operations supported by the [Lineage Integrator OMIS](/egeria-docs/services/omis/lineage-integrator).
...
The Lineage Integrator OMIS inside the integration daemon hosts the [integration connectors](/egeria-docs/concepts/integration=connector) that process the OpenLineage events.  They are divided in to two groups:
...
Figure 24 illustrates these mechanism with the five pre-build integration connectors supplied by Egeria.
...
3. The [Kafka OpenLineage Receiver](/egeria-docs/connectors/integration/kafka-open-lineage-receive-integraion-connector) integration connector is receiving OpenLineage events from the Kafka topic.  It passes them to the Lineage Integrator OMIS's context manager via its own context.
...
5. It creates OpenLineage events to represent this processing and passes it to the Lineage Integrator OMIS's context manager via its own context.
...
8. The [File-based OpenLineage Log Store](/egeria-docs/connectors/integration/file-based-open-lineage-log-store-integration-connector) registers a listener for OpenLineage events and stores each one received as a file in a nominated folder on the file system.

9. The [OpenLineage Cataloguer](/egeria-docs/connectors/integration/open-lineage-cataloguer-integration-connector) registers a listener for OpenLineage events and ensures the jobs describe in them are catalogued as [Processes](/egeria-docs/types/0/0010-Basic-Model/#process) in open metadata.  Depending on its configuration, it may also catalog each run as a TransientEmbeddedProcesses.
...
> **Figure 25:** An example deployment of Egeria that is capturing and processing OpenLineage events
