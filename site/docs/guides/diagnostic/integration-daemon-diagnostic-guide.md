...
```
GET {serverRootURL}/servers/{serverName}/open-metadata/integration-daemon/users/{userId}/restart
```
If you only want to restart a single connector, add the connector name to the request body.

Once the connectors are restarted, the status changes.  Disappointingly, there are still problems.
...
```
However, it is clear from the message identifier `BASIC-FILES-INTEGRATION-CONNECTORS-404-001` that the problem is being reported by the connector itself.  This means the integration daemon is now working ok even if the connectors are not.
...
* 
* All connectors
* All connectors in an integration group
* A single connector
...
```
POST {serverRootURL}/servers/{serverName}/open-metadata/integration-daemon/users/{userId}/integration-groups/{service-url-marker}/refresh
```
