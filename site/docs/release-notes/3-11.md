...
??? functional "Additional HTTP headers can now be passed to security plugins"
    ...
    Additional HTTP headers can now be passed to security plugins and propagated on federated queries.

    See [odpi/egeria#5700](https://github.com/odpi/egeria/issues/5700){ target=gh } for the implementation discussion.

    This new functionality allows developers to use whatever headers are needed for security checks. The configuration consists of just declaring the headers that are needed in the application properties. 

    For more technical details please check the dedicated [section](/guides/admin/configuring-the-omag-server-platform/#additional-http-headers).
