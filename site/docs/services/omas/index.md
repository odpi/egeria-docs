...
An open metadata access service is a REST API that runs in a [metadata access server](/concepts/metadata-access-server) and provides specific support for working with Open Metadata stored in the [Open Metadata Repositories](/concepts/open-metadata-repository) for one of the [Open Metadata Frameworks](/frameworks) running in either a [View Server](/concepts/view-server), [Engine Host](/concepts/engine-host) or [Integration Daemon](/concepts/integration-daemon).
