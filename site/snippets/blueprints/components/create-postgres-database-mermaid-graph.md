```mermaid
---
title: SolutionComponent - Create PostgreSQL Database from Open Metadata [1e13b511-e452-4fe4-a075-c0a5c021c051]
---
flowchart TD
...
**create-postgres-database (PostgreSQLGovernance)**"}
1==>|"Implemented By"|6
...
```
> Create PostgreSQL Database from Open Metadata: Create an open metadata element that represents a PostgreSQL database using the properties supplied on the request.  These properties are used to populate a standard template that includes the asset for the database and the connection.  The schemas, tables and columns are not cataloged by this component. (Extracted from 6.0-SNAPSHOT)
